...
;   image cube. Simulated 9MK flare is added to input image cube, then
...
function sensitivity_th9, ima, frame=frame
...
for xshift = -flare_peak[0], xshiftpos-1 do begin
   for yshift = -flare_peak[1], yshiftpos-1 do begin 
      ...
      si = 0.0006
      ai = add_flare9(ima, frame, scale=si, pix_size=pix_size, move=[xshift,yshift], erange=erange)
      ...
      while (maxdi GT 2*maxa) do begin   ;loop until max = 2*noflare_diff_max
         ...
         af = add_flare9(ima, frame, scale=si, pix_size=pix_size, move=[xshift,yshift], erange=erange)
