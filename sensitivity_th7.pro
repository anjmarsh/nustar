...
;   image cube. Simulated 7MK flare is added to input image cube, then
...
si = 0.01
ai = add_flare7(ima, frame, scale=si, pix_size=pix_size, erange=erange)
...
for xshift = -flare_peak[0], xshiftpos-1 do begin
   for yshift = -flare_peak[1], yshiftpos-1 do begin 
      ...
      si = 0.002  ;initial scaling 
      ai = add_flare7(ima, frame, scale=si, pix_size=pix_size, move=[xshift,yshift], erange=erange)
      ...
      while (maxdi GT 2*maxa) do begin   ;loop until max = 2*noflare_diff_max
         ...
         af = add_flare7(ima, frame, scale=si, pix_size=pix_size, move=[xshift,yshift], erange=erange)
